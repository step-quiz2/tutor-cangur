    ...
    with col_dialeg:
        with st.container(key="dialogue_panel"):
            ...
            if _verdict is not None:
                ...
                st.divider()
                _remaining = T.messages_remaining(state)
                _at_limit = (_remaining == 0)
                with st.form("message_form", clear_on_submit=True,
                    ...
                    student_text = st.text_area(
                        ...
                        placeholder=(
                            "Has esgotat els missatges per a aquest "
                            "problema. Pots demanar pista o respondre "
                            "amb 'Ja tinc la resposta'."
                            if _at_limit
                            else "Escriu aquí la teva frase"
                        ),
                        disabled=_at_limit,
                    )
                    # Comptador sempre visible: l'alumne ha de saber que
                    # té un sostre per poder dosificar els missatges.
                    if _at_limit:
                        st.caption(
                            f"⚠️ Has fet servir els {T.MAX_STUDENT_MESSAGES} "
                            "missatges. Demana pista o respon directament."
                        )
                    else:
                        st.caption(
                            f"Missatges restants: {_remaining} de "
                            f"{T.MAX_STUDENT_MESSAGES}"
                        )
